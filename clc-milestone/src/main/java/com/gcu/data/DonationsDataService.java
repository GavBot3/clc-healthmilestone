...
public class DonationsDataService implements DataAccessInterface<DonationsModel> {
	...
	public List<DonationsModel> findAll() {
		...
		try {
			...
			while (srs.next()) {
				...
				donations.add(new DonationsModel(srs.getInt("ID"), srs.getString("organ"),
						srs.getString("donation_date"), srs.getInt("user_ID")));
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public DonationsModel findByDonationID(int id) {
		// TODO Auto-generated method stub
		System.out.print(id);
		String sql = "SELECT * FROM donations WHERE ID = ?";
		DonationsModel donation = new DonationsModel();
		...
		try {
			...
			while (srs.next()) {
				// Create DonationsModel objects and add them to the donations list
				donation.setOrgan(srs.getString("organ"));
				donation.setDonation_date(srs.getString("donation_date"));
				donation.setID(id);
				donation.setUser_ID(srs.getInt("user_ID"));
	...
	public boolean create(DonationsModel t) {
		...
		try {
			...
			int rows = jdbcTemplateObject.update(sql, t.getOrgan(), t.getDonation_date(), t.getUser_ID());
